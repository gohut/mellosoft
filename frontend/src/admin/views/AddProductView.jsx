...
function buildInitialForm() {
  ...
  return {
    ...
    features: [
      ...
      "Available Thickness: 6 inch & 8 inch",
...
export default function AddProductView() {
  ...
  const handleSubmit = (e) => {
    ...
    const accessorySubcats = ["memory-foam-pillow", "latex-pillow", "fiber-pillow", "mattress-protector", "fitted-bedspread", "blanket-duvet", "travel-bed"];
    const bedFrameSubcats = ["wooden-bed-frame", "platform-bed"];

    const activeCatSlug = form.category || "";
    const activeMainCatId = form.mainCategoryId || "";
    const activeSubCat = form.subCategory || "";

    const isAcc = activeCatSlug === "accessories" || activeMainCatId === "CAT-ACCESSORIES" || accessorySubcats.includes(activeSubCat);
    const isBedFrame = activeCatSlug === "bed-frames" || activeMainCatId === "CAT-BED-FRAMES" || bedFrameSubcats.includes(activeSubCat);

    let parentCat, subCat;
    if (isBedFrame) {
      parentCat = "bed-frames";
      subCat = activeSubCat && bedFrameSubcats.includes(activeSubCat) ? activeSubCat : "wooden-bed-frame";
    } else if (isAcc) {
      parentCat = "accessories";
      subCat = activeSubCat || "memory-foam-pillow";
    } else {
      parentCat = "mattresses";
      subCat = activeSubCat || (activeCatSlug && activeCatSlug !== "accessories" && activeCatSlug !== "bed-frames" ? activeCatSlug : "ortho");
    }
    ...
    const newProduct = {
      ...
      mainCategoryId: isBedFrame ? "CAT-BED-FRAMES" : (isAcc ? "CAT-ACCESSORIES" : "CAT-MATTRESSES"),
      parentCategory: parentCat,
      parentCategoryId: parentCat,
      subCategory: subCat,
      subcategory: subCat,
      subcategoryId: subCat,
      category: isBedFrame ? subCat : (isAcc ? "accessories" : subCat),
                ...
                <div style={fieldGroup}>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const isAcc = selectedMainId === "CAT-ACCESSORIES" || mainCat?.slug === "accessories";
                      setForm((prev) => {
                        const isDefaultMattressPerk = prev.deliveryPerks?.length === 2 &&
                          prev.deliveryPerks[1]?.text?.includes("100-night");
                        const isDefaultAccPerk = prev.deliveryPerks?.length === 2 &&
                          prev.deliveryPerks[1]?.text?.includes("Warranty");
                        
                        let nextPerks = prev.deliveryPerks;
                        if (isAcc && isDefaultMattressPerk) {
                          nextPerks = [
                            { id: "perk-1", icon: "truck", text: "Free shipping on orders over ₹5,000" },
                            { id: "perk-2", icon: "shield", text: "Official Manufacturer Warranty & Easy Returns" }
                          ];
                        } else if (!isAcc && isDefaultAccPerk) {
                          nextPerks = [
                            { id: "perk-1", icon: "truck", text: "Free shipping on orders over ₹5,000" },
                            { id: "perk-2", icon: "check", text: "100-night trial with free pickups and full refunds" }
                          ];
                  ...
                  <button
                    ...
                    onClick={() => setForm((prev) => ({
                      ...
                      features: [
                        ...
                        `Available Thickness: ${(prev.variantsList || ["6 inch", "8 inch"]).join(" & ")}`,
                        "Layer Details: Multi-layer comfort design",
                        "100-Night Sleep Trial & Direct Manufacturer Warranty"
                      ]
                    }))}
